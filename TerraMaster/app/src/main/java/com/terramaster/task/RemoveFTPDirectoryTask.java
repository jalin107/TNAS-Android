...
public class RemoveFTPDirectoryTask extends WorkerThread {
    protected FTPFileItem fileItem;
    private OnSuccessListener listener;

    public RemoveFTPDirectoryTask(Context context, FTPFileItem item, OnSuccessListener listener) {
        super(context);
    ...
    public String onWorkInBackground() {
        ...
        try {
            ...
            boolean isSuccess = ftpHelper.removeDirectory(fileItem);
            if (isSuccess) {
                return "1";
            }
        } catch (Exception e) {
...
//	    taskDetail.setTaskType(DBKeys.TASK_DELETING_DIRECTORY);
//	    taskDetail.setTaskStatus(DBKeys.STATUS_COMPLETE);
//	    
//	    dbHelper.insertTask(taskDetail);
            new SuccessToast(mContext, mContext.getString(R.string.success_delete_directory)).show();
            if (listener != null) {
                listener.onSuccess();
            }
        } else {
